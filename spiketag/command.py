...
def report(binaryfile, probefile, nbits=32):
    ...
    if len(binaryfile) == 2:
        ...
        fig, ax = plt.subplots(
            1, 2, figsize=(25, 15), gridspec_kw={"width_ratios": [3, 1]}
        )
        sns.scatterplot(
            x=t,
            y=prb.ch_idx[ch],
        ...
        ax[0].set_facecolor((0.1, 0.1, 0.1, 0.0))
        ...
        nspks_img = ax[1].imshow(
            nspks.reshape(-1, prb.group_len), cmap=plt.cm.magma
        )  # ocean_r
...
def fpga(probefile):
    ...
    fpga.thres[:] = -360
    click.echo(
        "FPGA-NSP initiates, load fpga in ipython via `%load_ext spiketag` then check variable `fpga`"
    )
...
def cp(notebookfile):
    ...
    url = "https://raw.githubusercontent.com/lapis42/spiketag/master/notebooks/template"
...
@click.option(
    "--ttlport", prompt="/dev/ttyACM0, COM3, or None", default="None"
)
def bmi(gui, ttlport):
    # import os
    # os.remove("./fet.bin")
    ...
    if gui == "raster":
        ...
        if ttlport != "None":
            print(ttlport)
            gui = BMI_RASTER_GUI(
                fet_file="./fet.bin", t_window=10e-3, view_window=10, ttlport=ttlport
            )
        else:
